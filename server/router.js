...
router.post('/delete', function (req, res) {
    ...
    let readSql  = "SELECT * FROM users WHERE ?";
    let values = [
        {
            id: user.id
        }
    ...
    connection.query(readSql, values, function (err, resultRead) {
        if (err) throw err;
        console.log(resultRead);



        let deletefromUsersWHERE = "DELETE FROM users  WHERE ?";

        connection.query(deletefromUsersWHERE, values, function (err, result) {
            if (err) throw err;
            console.log("Number of records deleted: " + result.affectedRows);

            let deletedOrder = resultRead[0]['order'];
            console.log('order', deletedOrder);
            let updateOrder = "UPDATE users  SET `order`=`order`- 1 WHERE `order`>= ?";
            connection.query(updateOrder, [deletedOrder], function (err, result) {
                if (err) throw err;
